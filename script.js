...
/** Macro5: BezierX wave (using a combined sin/cos for demonstration) */
 ...
 *    Uses the 'edgeModulus' slider for threshold.
 */
    ...
    for (let x = 1; x < width - 1; x++) {
      ...
      for (let ky = -1; ky <= 1; ky++) {
        for (let kx = -1; kx <= 1; kx++) {
          ...
          gx += kernelX[ky+1][kx+1] * val;
          gy += kernelY[ky+1][kx+1] * val;
        }
      }
...
/** Macro8: Offset Summation (pixel + offset pixel) */
...
/** Macro10: Doubling effect (multiply each pixel by 2) */
...
/** Macro11: Quad-Tree (use the quadTreeDepth slider for maxDepth) */
function applyQuadTree() {
  ...
  img.onload = () => {
    ...
    // We'll keep threshold fixed (800). You can also add a slider if you like.
    const threshold = 800;
...
document.getElementById("processBtn").addEventListener("click", () => {
  ...
  const quadDepthNum = parseInt(quadDepthSlider.value, 10);
  const edgeValNum = parseInt(edgeModSlider.value, 10);
  const colorCheck = document.getElementById("colorShift").checked;

  alert(`Process with:
  primeOffset=${primeValNum},
  bezierIntensity=${bezValNum},
  quadTreeDepth=${quadDepthNum},
  edgeModulus=${edgeValNum},
  colorShift=${colorCheck}`);
});

// Toggle X/Y axis overlay
